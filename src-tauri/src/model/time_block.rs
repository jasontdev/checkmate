...
impl TimeBlock {
    fn create(connection: &Connection, new_time_block: &NewTimeBlock) -> Result<Self, Error> {
        let mut stmt = connection.prepare("INSERT INTO time_block (task_id, start, finish) VALUES (?1, ?2, ?3)")?;
        stmt.execute((&new_time_block.task_id, &new_time_block.start, &new_time_block.finish))?;
